...
| **`locale`**    | <code>string</code> | Locale to force in the Crisp web chat widget (ISO 639-1), eg. `en`, `fr`, `es`. Web only: native SDKs follow the device/app locale. |
| **`tokenID`**   | <code>string</code> | Unique token identifier for the user session continuity.                                                                            |
