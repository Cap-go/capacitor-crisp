    ...
    switch (color) {
      ...
      case "orange":
        Crisp.pushSessionEvent(
          new SessionEvent(name, SessionEvent.Color.ORANGE)
        );
      case "yellow":
        Crisp.pushSessionEvent(
          new SessionEvent(name, SessionEvent.Color.YELLOW)
        );
      case "green":
        Crisp.pushSessionEvent(
          new SessionEvent(name, SessionEvent.Color.GREEN)
        );
      case "purple":
        Crisp.pushSessionEvent(
          new SessionEvent(name, SessionEvent.Color.PURPLE)
        );
      case "pink":
        Crisp.pushSessionEvent(new SessionEvent(name, SessionEvent.Color.PINK));
      case "brown":
        Crisp.pushSessionEvent(
          new SessionEvent(name, SessionEvent.Color.BROWN)
        );
      case "grey":
        Crisp.pushSessionEvent(new SessionEvent(name, SessionEvent.Color.GREY));
      case "black":
        Crisp.pushSessionEvent(
          new SessionEvent(name, SessionEvent.Color.BLACK)
        );
      default:
    ...
    try {
      Crisp.setUserCompany(
        new Company(
          ...
          description,
          new Employment(
            employment.get(0).toString(),
            employment.get(1).toString()
          ),
          new Geolocation(
            geolocation.get(0).toString(),
            geolocation.get(1).toString()
          )
        )
      );
